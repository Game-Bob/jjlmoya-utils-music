...
export interface MusicToolEntry<TUI extends Record<string, string> = Record<string, string>> {
  id: string;
  icons: {
    bg: string;
    fg: string;
...
export interface MusicCategoryEntry {
  icon: string;
  tools: MusicToolEntry[];
  i18n: LocaleMap<CategoryLocaleContent>;
}

export interface ToolDefinition {
  entry: MusicToolEntry;
  Component: unknown;
